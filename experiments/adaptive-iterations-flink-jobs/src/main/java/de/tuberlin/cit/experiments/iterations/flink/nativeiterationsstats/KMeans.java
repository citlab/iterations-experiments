...
public class KMeans {
	...
	private static final String ACCUMULATOR_NAME = "KMeansReduceAccumulator";
	...
	public static void main(String[] args) throws Exception {
		...
		DataSet<Centroid> newCentroids = points
			...
			.groupBy(0).reduce(new CentroidAccumulator(operatorStatisticsConfig))
	...
	/** Sums and counts point coordinates while tracking statistics for this operator. */
	@ForwardedFields("0")
	public static final class CentroidAccumulator extends RichReduceFunction<Tuple3<Integer, Point, Long>> {
		...
		public CentroidAccumulator(OperatorStatisticsConfig config) {
		...
		public void open(Configuration parameters) {
			...
			globalAccumulator = runtimeContext.getAccumulator(ACCUMULATOR_NAME);
			if (globalAccumulator == null) {
				runtimeContext.addAccumulator(ACCUMULATOR_NAME, new OperatorStatisticsAccumulator(accumulatorConfig));
				globalAccumulator = runtimeContext.getAccumulator(ACCUMULATOR_NAME);
			}

			int subTaskIndex = runtimeContext.getIndexOfThisSubtask();
			localAccumulator = runtimeContext.getAccumulator(ACCUMULATOR_NAME + "-" + subTaskIndex);
			if (localAccumulator == null) {
				runtimeContext.addAccumulator(ACCUMULATOR_NAME + "-" + subTaskIndex, new OperatorStatisticsAccumulator(accumulatorConfig));
				localAccumulator = runtimeContext.getAccumulator(ACCUMULATOR_NAME + "-" + subTaskIndex);
			}
		}
